...
# the appropriate number of times.  For example, ONE applies f
# once. THREE applies f 3 times.  And ZERO applies f zero times.
...
# functions in this manner.  So a boolean function (BTRUE or BFALSE)
...
# number 'n' for being zero and return ONE if it is zero and THREE if
# is not, it would be written like this:
#
#     IS_ZERO.(n).(ONE).(THREE)
...
# one you need.  For example, given P above, LEFT and RIGHT works like
# this:
#
#    LEFT.(P)   # returns ONE
...
# different technique that is pretty straight forward.
...
# To tell if a is less than or equal to b, subtract b from a (a-b),
# and if the result is less than or equal to zero, then a <= b.  Since
# our number representation does not handle negative numbers, we have
# a weird case where a-b is 0 if b is larger than a.  This works out
# for us because then all we need to do is test for zero.

IS_LEQ = ->(a) { ->(b) { IS_ZERO.(SUB.(a).(b)) } }

# I actually want to use "<" in the next function.  We can derive the
# less than function from "<=" by reversing the arguments and applying
# a logical NOT.
...
# about no self-references means that we can't write MOD as a straight
...
# cases where a is less than b.  If you feed that function into the
# improver, you will get a function that works for a < 2b.  Feed that
# back into the improver and you get a modulus function that works for
# a < 3b.  Each new version is slightly better than the next.  The
# Y-Combinator takes the improver and turns it into the full version
# of modulus.
...
# HEAD returns the first item of a list.  Tail returns a list of all
# the remaining elements after the first.
...
# will do the same, but starting with FOLD.  FOLD is similar to Ruby's
# reduce.  It is called like this:
#
#     FOLD.(list).(initial).(f)
#
...
# We will build map from fold by using a function that conses f(item)
# onto the previous return value.  By initializing with the empty
# list, the result will be a list of f applied ot each item.
...
# result (n % 10).  To prevent the digits from being built in reverse
# order, we use a trick where we pass the "result so far" as an
# argument.  When the number is finished converting (n<10), we just
# return the cons of n on the "result so far".  Initializing the
# "result so far" with an empty list completes the trick.
#
# Finally, it is useful to point out that choosing an encoding where a
# number < 10 is its own character encoding simplifies the overall
# conversion.
